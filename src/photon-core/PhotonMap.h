...
public:
    ...
    void tracePhoton(const Scene &scene, const Ray &ray, const vec3 &power, int depth)
    {
        ...
        double continue_prob = 0.75; // I'm planning to make this material dependent
        ...
        if (diffuse_material) {
            ...
            // For non-diffuse materials, we can still have bounces but might not store photons
            // This is simplified, but you might want to handle specular materials differently
            
            // For now, just use a default bounce with reduced energy
            double r1 = static_cast<double>(rand()) / RAND_MAX;
            double r2 = static_cast<double>(rand()) / RAND_MAX;
