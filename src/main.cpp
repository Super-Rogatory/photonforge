...
void visualizePhotons(Scene &scene, const PhotonMap &photonMap)
{
    ...
    double photonSize = 0.1; // Size of visualization spheres

    // Limit number of visualized photons to avoid overwhelming the scene
    int maxVisualized = 500;
...
int main()
{
    ...
    auto areaLight = std::make_shared<PointLight>(vec3(2.0, 5.0, 0.0), vec3(1.0, 1.0, 1.0), 12.0);
    scene.addLight(areaLight);
    scene.prepareLights();

    // ---- Build Photon Map ----
    // std::cout << "Building photon map..." << std::endl;
    // PhotonMap photonMap(500, 3); // 500 photons, 5 max bounces
    // photonMap.buildPhotonMap(scene, 500);
    ...
    // Render
    PathTracer tracer(256, 256, 100, 5);
